...
int main(int argc, string argv[]) // Оголошуєм у функції аргумент argc та масив стрічки argv[] 
{
  ...
  {
   if (!isalpha(key[k]))        //Перевіряємо чи к-елемент масиву key є літерою
    {
     printf("Try again.");
     return 1;
    }
   ...
   {
      if (isalpha(text[i])) //Якщо елементи масиву є літерами
    {
      if (isupper(text[i]))                                                                   
          printf("%c", ((((text[i] - 'A') + ((toupper(key[n++%keyCount]))-'A')%26) % 26) + 'A'));  // Робимо маленькі літери великими
